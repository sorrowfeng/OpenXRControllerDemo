...
- `Mapping` 枚举仍保留在代码里做兼容，但主导航不再单独暴露这一页
- 映射信息已经合并进 `手势` 页
- `主页` 现在是“状态 + JSON”中心布局，不再是原来的总览拼接页
...
- 左侧小状态边栏
- 中央最大区域显示左右手分栏 JSON
- `左手 JSON / 右手 JSON` 标题已经压小
- 状态卡现在是整块背景随状态变色，并保留副标题文字
...
主页当前已经不是“说明文字驱动”的总览，而是直接围绕 UDP 正在发出的源数据组织：

- JSON 会格式化显示
- `hands` 数组会拆成左右手两栏分别显示
- 主页顶部状态卡会根据状态整块变色
...
- `manual_target_ip` 会保存
- `remembered_target_ip` 会保存
- `udp_port` 会保存
- `send_rate_hz` 会保存
...
- 不再只依赖 3D 射线本体
- 会直接在 GUI 窗口表面绘制高对比度落点准星
- 准星绘制已放到 ImGui 前景层，避免被按钮和子窗口遮挡
- 当前激活射线与按下触发时会有更明显的颜色变化
...
- `assembleDebug` 可以通过
- APK 可以安装到设备
- 多次 `adb install -r` 已成功
- `NativeActivity` 可以被成功拉起
- 最新版 `200Hz` 持续发送已实机验证有效
- 主页 JSON 中心化布局版本可以构建并安装
- 主页顶部状态卡变色版本可以构建并安装
- `B` 键独立切换窗口显隐版本可以构建并安装
- 之前的若干运行日志里出现过：
  - 中文字体成功加载
  - GUI draw list
